...
export default function SubPageSideBar({onMenuClick}) {
  ...
  return (
    <div>
      ...
      <CgMenu className={` text-seconderyColor text-3xl rounded absolute -right-28 top-[25px] cursor-pointer1`} onClick={() => setCollapsed(!collapsed)} />
        <ul>
          {subNavlinks.map((submenu, index) => (
            <Link onClick={() => onMenuClick(submenu.name)} to={submenu.link} className={`group rounded-md flex flex-col items-center text-center align-middle gap-2`}>
              <li key={index} />
              <img className=" w-8 iconColor text-center text-[#727272]" src={submenu.icon} />
              <span className={`pl-1 text-sideBarTextColor`}>{submenu.name}</span>
            </Link>
          ))}
        </ul>
      </div>
